-- =========================================================================
-- 🛡️ Security Patch & Logic Fix: Return has_more_pending flag
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Check if we hit the 36 iterations cap but there are still pending runs
    IF v_rec.next_run_date <= CURRENT_DATE AND (v_rec.end_date IS NULL OR v_rec.next_run_date <= v_rec.end_date) THEN
      v_has_more_pending := TRUE;
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'has_more_pending', v_has_more_pending
  );
END;
