...
import { 
  fallbackBuckets, 
  calculateMonthlyTransactionsSummary, 
  getWalletsWithFallback, 
...
export default async function DashboardPage() {
  ...
  // 3. Fetch Buckets
  let { data: bucketsData } = await supabase.from('buckets').select('*').order('created_at')
  const buckets = (!bucketsData || bucketsData.length === 0) ? fallbackBuckets : bucketsData

  // 4. Fetch Monthly Transactions
  ...
  const { monthIncome, monthExpense, bucketMonthlyExpenses } = calculateMonthlyTransactionsSummary(monthTransactions)

  // 5. Fetch Wallets
  let { data: walletsData } = user ? await supabase
    .from('wallets')
    .select('*')
    .eq('user_id', user.id)
  ...
  const wallets = getWalletsWithFallback(walletsData, buckets, user?.id || '')

  // 6. Calculate derived data
  const totalBalance = calculateTotalBalance(wallets, buckets)
  const budgetAlerts = calculateBudgetAlerts(buckets, bucketMonthlyExpenses)
